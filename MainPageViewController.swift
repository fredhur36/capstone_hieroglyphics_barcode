...
class MainPageViewController: UIViewController {
    ...
    init(){
        ...
        let captureSession = cameraViewController.getCaptureSession()
        pagingViewController = FixedPagingViewController(viewControllers: [ScreenViewController(index: 0, name : "scan", session : captureSession), ScreenViewController(index: 1, name : "reigster", session : captureSession)])
    ...
    func photoOutput(_ ouput: AVCapturePhotoOutput, didFinishProcessingPhoto photo : AVCapturePhoto, error: Error?){
        if let imageData = photo.fileDataRepresentation(){
            ...
            if(currentIndex == 0 ){
                ...
                scanModeViewController = ScanModeViewController(image : image!)
                self.present(scanModeViewController, animated: true, completion: nil) // need to change this to registerModeCcontroller
